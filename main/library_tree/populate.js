...
class Populate {
	constructor() {
		...
		this.cur = [];
		this.cur_ix = 0;
		this.customSort = ''; // Regorxxx <- Expand TF support. Fix sorting not being applied after HTML options panel change ->
	...
	setTf() {
		this.tf = {
			...
			custom1MeX: FbTitleFormat(ppt.tfCustom1MeX),
			custom2MeX: FbTitleFormat(ppt.tfCustom2MeX),
		...
		switch (this.clicked_on) {
			...
			case 'text':
				...
				if (panel.isStandardSource()) {
					if (this.dblClickAction <= 1 && !this.dblClickAction && !this.autoFill.mouse && !this.autoPlay.click) {
						return this.send(item, x, y);
					}
					...
					if (ppt.plsActions && this.dblClickAction !== 1) {
						if (this.dblClickAction === 0 && !this.autoFill.mouse && !this.autoPlay.click) {
	...
	lbtn_up(x, y) {
		...
		// Regorxxx <- Mouse actions on playlist sources
		if (panel.isStandardSource()) {
			if (vk.k('alt')) { return this.mbtnUpOrAltClickUp(x, y, '', 'alt'); }
			...
			if (ppt.plsActions) {
				if (vk.k('alt')) {
					if (ppt.actionMode === 2) {
						...
						return this.mbtnUpOrAltClickUp(x, y, '', 'alt');
					}
				}
				if (this.autoFill.mouse || this.autoPlay.click) {
					this.setPlaylistSelection(ix, item);
	...
	load({ handleList, bAddToPls = false, bAutoPlay = false, bUseDefaultPls = false, bInsertToPls = false, bApplySort = true } = {}) {
		...
		const items = bApplySort
			? this.sortIfNeeded(!handleList || Array.isArray(handleList) ? this.getHandleList() : handleList.Clone())
			: !handleList || Array.isArray(handleList) ? this.getHandleList() : handleList.Clone();
		// Regorxxx ->
	...
	mbtnUpOrAltClickUp(x, y, mask, type) {
		...
		if (isForcedPlsMiddleAction || (type == 'mbtn' && (ppt.actionMode == 2 || ppt.mbtnClickAction == 2) || type == 'alt' && ppt.altClickAction == 2) && !isForcedPlsAltAction) {
			...
			if (panel.isStandardSource()) {
				...
				if (ix < this.tree.length && ix >= 0) {
					const item = this.tree[ix];
					if (isForcedPlsAltAction || [4, 6].includes(ppt[`${type}ClickAction`]) || panel.isDefaultPlaylistSource() || this.isPlaylistParentDefaultPlaylist(item)) {
						this.setPlaylistSelection(ix, item, void (0), this.getTopTracks);
					} else {
	...
	on_key_down(vkey) {
		...
		switch (vkey) {
			...
			case vk.end:
				if (this.tree.length == 0) break;
				if (vk.k('alt')) { return this.on_key_down(vk.dn); } // Regorxxx <- Jump to fist/last sibling ->
			...
			case vk.up:
				...
				this.row.i = -1;
				this.m.i = -1;
				// Regorxxx <- Jump to fist/last sibling
				if (vk.k('alt')) {
					item = this.tree[panel.pos];
					let sibling;
					if (panel.imgView) {
			...
			switch (type) {
				...
				case 'focus': {
					const delta = (img.style.vertical ? img.panel.h : img.panel.w) / 2 > sbar.row.h
						? Math.floor((img.style.vertical ? img.panel.h : img.panel.w) / 2)
						: 0;
