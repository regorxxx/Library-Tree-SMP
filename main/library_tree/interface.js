...
class UserInterface {
	constructor() {
		...
		this.img = {
			...
			blurAlpha: $.clamp(ppt.blurAlpha, 0, 100) / 30,
			blurLevel: 100,
			covAlpha: $.clamp(ppt.covAlpha * 2.55, 0, 255),
			cur: null,
			cur_pth: '',
			isBlur: false,
			stub: [],
		...
		this.sz = {
			...
			marginSearch: 0,
			node: Math.round(11 * $.scale),
			node_base: Math.round(11 * $.scale),
			pad: 1,
			sel: 3,
			sideMarker: 4,
			sp: 6,
	...
	assignColours() {
		...
		propRGB.forEach((v) => this.col[v] = set(ppt[v + 'Use'] ? ppt[v] : '', 'rgb'));
		propRGBA.forEach((v) => this.col[v] = set(ppt[v + 'Use'] ? ppt[v] : '', 'rgba'));
	}
	// Regorxxx ->
	...
	draw(gr) {
		if (this.style.bg) gr.FillSolidRect(0, 0, this.w, this.h, this.col.bg);
		if (this.img.isBlur || this.img.bg) {
			this.getImgFallback();
			if (this.img.cur) gr.DrawImage(this.img.cur, 0, 0, this.w, this.h, 0, 0, this.img.cur.Width, this.img.cur.Height);
		}
	}

	drawLine(gr) {
		if (!this.style.topBarShow) return;
		if (this.style.pen == 1) gr.DrawLine(panel.ln.x, panel.search.sp, panel.ln.w, panel.search.sp, this.l.w, this.col.s_line);
	...
	getFont(init) {
		...
		this.sz.node = this.style.squareNode ? Math.round(this.sz.node_base * ppt.zoomNode / 100) : Math.round(ppt.baseFontSize * ppt.zoomNode / 100);
	...
	getItemColours() {
		...
		this.col.imgOverlaySel = $.RGBtoRGBA(this.col.bg, 175);
	...
	isLightBackground() {
		...
		if (ppt.themed && (ppt.theme == 0 || ppt.theme == 6 || ppt.theme == 7) && this.themeColour && ppt.themeColour) {
			...
			if (this.img.blurLight) isLightBg = true;
			else if (this.img.blurDark) isLightBg = false;
		}
		return isLightBg;
	}
	...
	setIconCol() {
		...
		if (this.style.squareNode) {
			if (this.getAlpha(this.icon.col_h) != 255) {
				this.col.icon_h = $.RGBAtoRGB(this.icon.col_h, this.col.bg);
	...
	wheel(step, all) {
		if (!vk.k('shift')) {
			const textZoom = panel.m.x >= Math.round(this.icon.w + this.sz.margin + (ppt.rootNode && !pop.inlineRoot ? ppt.treeIndent : 0));
			if (panel.m.y > panel.search.h && textZoom || all) this.zoomText(step);
			if (panel.m.y > panel.search.h && !textZoom || all) this.zoomNode(step);
			if (panel.m.y <= panel.search.h || all) this.zoomFilter(step);
			img.sizeDebounce();
		}
		if (vk.k('shift') && (panel.m.y > panel.search.h || all)) this.imageZoom(step);
		window.Repaint();
	}
