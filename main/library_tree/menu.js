...
class MenuManager {
	...
	load(x, y) {
		...
		while (i < ln) {
			...
			v.appendMenu ? this.appendMenu(v) : this.addItem(v);
			i++;
		}
...
class MenuItems {
	...
	mainMenu() {
			...
			if (panel.isQueueSource()) {
				...
				// Regorxxx <- Show previously playing. Check if item is tracked
				{
					...
					menu.newItem({
						...
						flags: item === -1 ? MF_GRAYED : MF_STRING
					});
				}
				// Regorxxx ->
			}
		...
		if ((this.validItem || !this.settingsBtnDn && ppt.settingsShow) && (panel.isAllPlaylistSource(true) || panel.isFixedPlaylistSource()) && !ppt.plsFlatView) {
			const parent = this.validItem ? pop.lastSelMul.flatMap((idx) => pop.getPlaylistParent(pop.tree[idx])) : [];
			const isValidPls = parent.length === 1 && parent[0].idx !== -1;
			const isMultiplePls = parent.length > 1;
			const locks = isMultiplePls ? parent.map((p) => getLocks(p.idx)) : [getLocks(isValidPls ? parent[0].idx : -1)];
			const names = parent.map((p) => p.name);
			const menuName = !this.validItem || isMultiplePls ? 'Playlists' : 'Playlist';
			if (this.validItem) { menu.newItem({ separator: true }); }
			...
			if (this.validItem) {
				...
				menu.newItem({ menuName, separator: true });
				{
					...
					const flags = locks[0].isSMPLock ? MF_STRING : MF_GRAYED;
					const subMenuName = 'Lock';
					menu.newMenu({ menuName: subMenuName, appendTo: menuName, flags: locks[0].types.length ? MF_CHECKED : MF_STRING });
					menu.newItem({ menuName: subMenuName, str: 'Lock by action:' + (locks[0].isSMPLock ? '' : '\t' + _p(locks[0].name)), flags: MF_GRAYED });
					...
					lockTypes.forEach((lock) => {
						menu.newItem({
							...
							flags,
							checkItem: locks[0].types.includes(lock.type)
						});
					});
					menu.newItem({ menuName: subMenuName, separator: true });
					menu.newItem({
			...
			menu.newItem({ menuName, separator: true });
			{
				...
				menu.newItem({
					menuName: subMenuName,
					str: 'Load Playlist...',
					func: () => fb.LoadPlaylist()
				});
				menu.newItem({ menuName: subMenuName, separator: true });
				menu.newItem({
					menuName: subMenuName,
					str: 'New AutoPlaylist...',
					func: () => {
			...
			{
				const subMenuName = 'Restore';
				menu.newMenu({ menuName: subMenuName, appendTo: menuName, flags: plman.PlaylistRecycler.Count > 0 ? MF_STRING : MF_GRAYED });
				const deletedItems = [];
				for (let i = 0; i < plman.PlaylistRecycler.Count; i++) { deletedItems.push(plman.PlaylistRecycler.GetName(i)); }
				deletedItems.slice(0, 8).forEach((str, i) => {
					menu.newItem({
						menuName: subMenuName,
						str: str,
						func: () => plman.PlaylistRecycler.Restore(i)
					});
				});
			}
			menu.newItem({ menuName, separator: true });
			{
				const subMenuName = 'Other tools';
				menu.newMenu({ menuName: subMenuName, appendTo: menuName });
				if (this.validItem) {
					const deadItemsArr = [];
					let bLocked = true;
					parent.forEach((p, i) => {
						deadItemsArr.push(
							plman.PlaylistItemCount(p.idx) < 20000
								? countDeadItems(p.idx)
								: '?'
						);
						if (deadItemsArr.at(-1) !== 0) {
							bLocked = bLocked && ['AddItems', 'RemoveItems'].some((l) => locks[i].types.includes(l));
						}
					});
					...
					menu.newItem({
						menuName: subMenuName, str: 'Revive dead items\t' + (bLocked ? '[locked]' : _b(deadItems)), func: () => {
							parent.forEach((p) => playlistRevive({ playlist: p.idx, simThreshold: 0.5, bFindAlternative: true, bNotifyPlsMan: false }));
						}, flags: !bLocked && deadItems ? MF_STRING : MF_GRAYED
					});
					menu.newItem({
						menuName: subMenuName, str: 'Select dead items\t' + _b(deadItems), func: () => {
							parent.forEach((p) => selectDeadItems(p.idx));
						}, flags: deadItems ? MF_STRING : MF_GRAYED
					});
				} else {
					menu.newItem({ menuName: subMenuName, str: 'Revive dead items\t[-N/A-]', flags: MF_GRAYED });
				}
			}
			if (this.validItem) {
				menu.newItem({ menuName, separator: true });
				menu.newItem({
					...
					flags: isValidPls && !locks[0].types.includes('RemovePlaylist') || isMultiplePls ? MF_STRING : MF_GRAYED
				});
			}
		}
		// Regorxxx ->
		...
		// Regorxxx <- Auto-DJ feature
		{
			...
			if (!panel.isAutoDjSource()) {
				menu.newItem({
					...
					func: () => panel.startAutoDj(this.items),
					flags: this.items.Count > 1 ? MF_STRING : MF_GRAYED,
					checkItem: !!(panel.autoDj.running && panel.autoDj.source)
				});
				menu.newItem({
					menuName: 'Auto-DJ',
			...
			if (panel.isAutoDjSource()) {
				menu.newItem({
					...
					func: () => this.setCachedSource(0, void (0), 'Prev. Source', { bOmitMsg: true, bSkipPresets: true }), // Regorxxx <- Internal cache of views | Preset rules ->
					flags: MF_STRING
				});
			} else {
				menu.newItem({
					menuName: 'Auto-DJ',
					str: 'Show queue...',
					func: () => this.setCachedSource(4, 'Prev. Source', void (0), { bOmitMsg: true, bSkipPresets: true }), // Regorxxx <- Internal cache of views | Preset rules ->
		...
		// Regorxxx <- Quick help | External integration | Preset rules
		{
			...
			menu.newItem({
				...
				func: () => {
					fb.ShowPopupMessage(
						$.open(folders.xxx + 'helpers\\readme\\library_tree_presets.txt'),
						window.PanelName + ': Preset rules'
					);
				}
			});
			menu.newItem({ menuName: 'Help', separator: true });
		...
		panel.menu.forEach((v, i) => {
			...
			if (bSeparator) {
				...
				menu.newItem({
					menuName: appendTo ? 'Views' : void (0),
					str: v,
					func: () => this.setView(i, { bSkipPresets: vk.k('shift') }), // Regorxxx <- Preset rules ->
	...
	getPaylistFlag(i) {
		...
		const lock = getLocks(plnIsValid ? pln : -1);
		const plLockAdd = plnIsValid
			? lock.types.includes('AddItems')
			: false;
		const plLockRemoveOrAdd = plnIsValid
			? lock.types.includes('RemoveItems') || lock.types.includes('ReplaceItems') || plLockAdd
			: false;
	...
	getSortData(d) {
		...
		while (l-- && l) {
			d.value = d.value.replace(new RegExp($.regexEscape(d.sortYear[l]), 'gi'), ''); // Regorxxx <- Sorting identification should not be case-sensitive ->
		...
		if (d.sortIX == -1) {
			...
			while (l-- && l) {
				d.value = d.value.replace(new RegExp($.regexEscape(d.sortAlbumsByYearAfter[l]), 'gi'), '%ALBUM%'); // Regorxxx <- Sorting identification should not be case-sensitive ->
				if (d.valueLength != d.value.length) {
					d.sortIX = l;
					d.valueLength = d.value.length;
				}
		...
		if (d.sortIX == -1) {
			...
			while (l-- && l) {
				d.value = d.value.replace(new RegExp($.regexEscape(d.sortAlbumsByYearBefore[l]), 'gi'), '%ALBUM%'); // Regorxxx <- Sorting identification should not be case-sensitive ->
				if (d.valueLength != d.value.length) {
					d.sortIX = l;
					d.valueLength = d.value.length;
				}
	...
	rbtn_up(x, y, settingsBtnDn) {
		...
		let row = -1;
		const level = pop.tree.length > this.ix && this.ix != -1 ? pop.inlineRoot ? Math.max(item.level - 1, 0) : item.level : -1;

		this.validItem = this.settingsBtnDn ? false : panel.imgView ? pop.tree.length > this.ix && this.ix != -1 : y < panel.tree.y + pop.rows * sbar.row.h && pop.tree.length > this.ix && this.ix != -1 && (x < Math.round(ppt.treeIndent * level) + ui.icon.w + ppt.margin && (!item.track || item.root) || pop.check_ix(item, x, y, true));

		if (!this.validItem && !this.settingsBtnDn && ppt.settingsShow && y > panel.search.sp) {
			this.ix = pop.row.i == -1 ? panel.imgView ? -1 : pop.tree.length - 1 : pop.row.i;
	...
	setSource(i, { bOmitMsg = false, bProcessTree = true, bSkipPresets = false } = {}) {
		if (panel.isFileExplorerSource() && i !== 5) { explorer.exit(); but.refresh(); } // Regorxxx <- File explorer mode ->
		switch (i) {
			case 0: // Library
				ppt.libSource = 1;
				ppt.fixedPlaylist = false;
	...
	sortByDate(i, d) {
		...
		if (d.sortType == 1 || sortByIX != -1) {
			const expanded = [];
			const ix = pop.get_ix(panel.imgView ? img.panel.x + 1 : 0, (!panel.imgView || img.style.vertical ? panel.tree.y : panel.tree.x) + sbar.row.h / 2, true, false);
			const curName = ix == -1 ? '' : pop.tree[ix].name;
			const scrollPos = sbar.scroll;
			const selected = [];
			pop.tree.forEach((v, i) => {
				const level = ppt.rootNode ? v.level - 1 : v.level; // 1 level memory: more is less reliable
				if (!level) {
			...
			pop.clearTree();
			panel.getViews();
			this.setView(ppt.viewBy, { bSkipPresets: true }); // Regorxxx <- Preset rules ->
