...
## [Unreleased][]
### Added
- Syntax: added special Library-Tree-SMP syntax docs which can be opened via 'search menu\Help\Library-Tree-SMP syntax'. These docs show all special functions using on views, filter, search box and sorting. Also added a link to this file at the HTML options panel at relevant places; note that your browser may fail opening it due to security settings, but the link may be copied and pasted on any web/file browser to open it locally. <ins>[new]</ins>
- Auto-DJ: added new Auto-DJ feature which will randomly take tracks from the tree based on the current view and add them to the queue. Since it takes tracks from current view, whatever affects the tracks shown on panel will also affect Auto-DJ (filters, search box,...). The algorithm tries to never take the same track 2 times, if possible, and stops whenever playback is manually stopped or if no more tracks are found. To fully make use of this feature is recommended to enable the global duplicates removal filter, so tracks with equal tags are also skipped (for ex. to avoid live and studio versions of same tracks). Additionally, using a dynamic filter based on playback will allow Auto-DJ to keep adding new tracks to the queue based on what was played before, similar to how Spotify works ('Nowplaying similar' default filter would be ideal for this use case); an specific query at the search input box would also greatly help to ensure the listening session is limited to specific dates, genres, etc. <ins>[new]</ins>
...
- Filters: added new default filter query expressions like: 'Nowplaying Similar Artists' (*), 'Nowplaying Genre', 'Nowplaying Style',... (*) Needs previous tagging with other scripts like [Infinity-Tools-SMP](github.com/regorxxx/Infinity-Tools-SMP). [from Library-Tree-v2.4.0.mod.21]
- Filters: added global duplicates removal filter, with customizable TF expressions, which can be applied in top of any View and Filter setting. <ins>[new]</ins>
...
- Top tracks: sorting TF supports $harmonicsort{}, $harmonicmix{} and $shufflebytags{tagName,sortBias,sortDir}. <ins>[new]</ins>
	* $harmonicsort{bShuffle}: argument is optional, if empty or set to any string (except 'false' and '0'), shuffles the tracks first. Performs sorting by incremental KEY.
		º $harmonicsort{true}, $harmonicsort{1}, $harmonicsort{abc}, $harmonicsort{} are equivalent.
		º $harmonicsort{false} and $harmonicsort{0} are equivalent (not shuffled).
	* $harmonicmix{bShuffle}: argument is optional, if set to any string except 'false', shuffles the tracks first. Performs sorting by harmonic mixing rules.
	* $shufflebytags{tagName,sortBias,sortDir}: requires only 'tagName' argument. Applies semi-random patterns, not allowing the same tag 2 times in a row, while not falling into strict intercalation. The sorting bias is used to prefer tracks by specific conditions.
	...
	* Conditional execution can be partially achieved, but note the special sorting functions are evaluated only once (for all tracks). This means that TF expressions based on tags will probably not work as expected except when evaluating against playback or selection. For conditional execution based on player state is better to use global variables like '%ISPLAYING%'', '%ISPAUSED%', ... For example: '$if(%ISPLAYING%,$harmonicmix{})'. Check Library-Tree-SMP Title format reference for more info.
...
- Statistics: added 6 customizable slots statistics, 3 for averaged values and 3 for summed ones. it uses the decimals setting (see above). Slots can be modified easily at the HTML options panel or at 'Statistics Titleformat Custom-X (sum)' and 'Statistics Titleformat Custom-X (avg)' properties, and their labels at 'Statistics Titleformat Custom labels' property. By default the panel includes custom expressions as example. [from Library-Tree-v2.4.0.mod.21]
...
- Debug: added profiling logging for library loading (properties panel). Enabled by default. [from Library-Tree-v2.4.0.mod.21]
...
- Filters: all default filter presets using $nowplaying{} have been tweaked to use $nowplayingorselected{} instead, so they always work with selection as fallback. Restore filter defaults to use them. [from Library-Tree-v2.4.0.mod.21]
