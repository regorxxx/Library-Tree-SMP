...
- Sources: new Auto-DJ queue source, which shows all tracks currently on Auto-DJ cache. Note it only works with non-live view sources, since otherwise 'the source would be itself'. Drag n' drop is supported to add new tracks to the list, along contextual menu entry to add (from other sources) or remove tracks from it. While Auto-DJ is running, you can switch back and forth between any source and Auto-DJ queue without losing the cache. The new source can be accessed through the usual source submenu or the Auto-DJ one.
...
- Auto-DJ: new setting to set how the next track is chosen: randomly, by genre-mood, genre or mood tags matching.
